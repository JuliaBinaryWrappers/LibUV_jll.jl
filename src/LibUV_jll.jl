...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import LibUV_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("LibUV_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "LibUV_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"LibUV", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "LibUV_jll", "src", "LibUV_jll.jl"))
        @info("LibUV_ll dev'ed out to /depot/dev/LibUV_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("183b4373-6708-53ba-ad28-60e28bb38547"))
platforms = [Pkg.Artifacts.unpack_platform(e, "LibUV", artifacts_toml) for e in artifacts["LibUV"]]
...
if best_platform === nothing
    @debug("Unable to load LibUV; unsupported platform $(triplet(platform_key_abi()))")
else
